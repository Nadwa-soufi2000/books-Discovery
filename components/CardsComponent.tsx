...
export function CardsComponent({ author_name, cover_i, first_publish_year, title }: bookObject) {
  const coverSrc = cover_i
    ? `https://covers.openlibrary.org/b/id/${cover_i}-M.jpg`
    : "/placeholder-book.png";

  return (
    <Card className="h-full min-h-[420px] w-full sm:w-[200px] md:w-[200px] lg:w-[250px] h-[460px] flex flex-col">
      <CardContent className="flex h-full flex-col items-center justify-between p-4">
        ...
        <span className="mt-3 text-3xl font-semibold">{first_publish_year}</span>
      </CardContent>
    </Card>
  );
}
